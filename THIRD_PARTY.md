...
### kubridge import stub (`third_party/kubridge-stub`)

The JIT pool asks the [kubridge](https://github.com/bythos14/kubridge)
kernel plugin (TheOfficialFloW / bythos14) for executable memory beyond the
kernel's 16 MiB VM quota, for page protection (guard pages, SMC barrier) and
for a user-mode abort handler (`mman_vita.c`, `fault_vita.c` in the engine). The plugin itself is
**not** bundled and is optional at runtime (weak stub, detected with
`_vshKernelSearchModuleByName`, 16 MiB fallback). Neither upstream
repository carries a license file, so nothing of the plugin is copied:
`stubs.yml` lists seven function names with their NIDs (the import interface,
regenerated by `regen.sh` with VitaSDK's `vita-libs-gen`), and the engine
declares those prototypes itself (`kubridge_min.h`).
